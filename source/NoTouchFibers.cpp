...
//    G4Box *solidWorld = new G4Box("solidWorld", 0.5*m + 1*cm, 2.0*cm, 0.5*m + 1*cm);

    G4Box *solidWorld = new G4Box("solidWorld", 0.5*m + 1.0*mm, 1.55*cm, 0.5*m + 1.0*mm);
...
//even that it's less than 1% of the photons that come out.
    ...
    G4LogicalSkinSurface *skin = new G4LogicalSkinSurface("skin",BiglogicWorld,mirrorSurface);
