...
//Small World Volume (cover)
    G4Box *solidWorld = new G4Box("solidWorld", 0.5*m + 3*cm, 4.0*cm, 0.5*m + 3*cm);
...
//AirGap for fibers
    ...
    auto*AirGap = new G4Box("AirGap",fiberradius/2.,fiberradius/2.,Scintlenght/2. + 0.15*cm);

    auto*LogicAirGap = new G4LogicalVolume(AirGap,worldMat,"LogicAirGap");
...
/////////////////////////////Translate of Fibers to not touch the plastic
    double moveY = fiberradius/2.0;

    G4VPhysicalVolume*PhysicAirGap[FiberNumber];
    for(int i=0;i<FiberNumber;i++){
    PhysicAirGap[i] = new G4PVPlacement(0,{-Scintlenght/2+gap/2+(i*gap),Scintwidth/2 + moveY,0.},LogicAirGap,"PhysicAirGap",logicWorld,false,0,true);
    }
